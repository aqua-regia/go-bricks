...
import (
	"bitbucket.org/coinswitch/go-bricks/redis_lock"
	"context"
	"fmt"
...
func Example() {
	...
	if err == redis_lock.ErrNotObtained {
...
func ExampleClient_Obtain_retry() {
	...
	locker := redis_lock.New(client)

	ctx := context.Background()

	// Retry every 100ms, for up-to 3x
	backoff := redis_lock.LimitRetry(redis_lock.LinearBackoff(100*time.Millisecond), 3)

	// Obtain lock with retry
	lock, err := locker.Obtain(ctx, "my-key", time.Second, &redis_lock.Options{
...
func ExampleClient_Obtain_customDeadline() {
	...
	backoff := redis_lock.LinearBackoff(500 * time.Millisecond)
