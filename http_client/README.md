...
`http_client` wraps Go's standard lib http client to provide several options to increase resiliency in your request. If you experience poor network conditions or requests could experience varied delays, you can now http_client the endpoint for data.
- Send out multiple requests and get the first back (only used for GET calls)
- Retry on errors
- Backoff

### Simple Example
Use `http_client` where you would use the http client calls. By default, http_client will use a concurrency of 1, and retry the endpoint 3 times with the `DefaultBackoff` strategy of waiting 1 second between retries.
```go
/* swap in replacement, just switch
   http.{Get|Post|PostForm|Head|Do} to
   http_client.{Get|Post|PostForm|Head|Do}
*/
resp, err := http_client.Get("http://sethammons.com")
```
...
`http_client` allows you to use a constructor to control:
- backoff strategy
- retries
- concurrency
...
func main() {
    ...
    { // drop in replacement for http.Get and other client methods
        resp, err := http_client.Get("http://example.com")
        if err != nil {
    ...
    { // control the resiliency
        ...
        client.MaxRetries = 5
        client.Backoff = http_client.ExponentialBackoff
        client.KeepLog = true
    ...
    { // use the http_client version of http.Client.Do
...
### Example Log
`http_client` also allows you to control the resiliency and can optionally log the errors.
```go
c := http_client.New()
c.KeepLog = true
...
You can see `http_client` in action with `$ cd sample; go run main.go`.

For watching open file descriptors, you can run `watch "lsof -i -P | grep main"` if you started the app with `go run main.go`.
I did this for watching for FD leaks. My method was to alter `sample/main.go` to only run one case (`http_client.Get with set backoff stategy, concurrency and retries increased`)
